...
public:
    int longestCommonSubsequence(string text1, string text2) {
        ...
        // Create a 2D array to store lengths of common subsequence at each
        // index.
        ...
        for (int i = 1; i <= text1Length; ++i) {
            for (int j = 1; j <= text2Length; ++j) {
                // If current characters match, add 1 to the length of the
                // sequence until the previous character from both strings.
                if (text1[i - 1] == text2[j - 1]) {
                    dp[i][j] = dp[i - 1][j - 1] + 1;
                } else {
                    // If current characters do not match, take the maximum
                    // length achieved by either skipping the current character
                    // of text1 or text2.
